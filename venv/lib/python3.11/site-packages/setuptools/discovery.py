...
- "app-layout": a directory representing a Python project that contains a "app"
  folder. Everything under the "app" folder is meant to be included in the
...
- "flat-layout": a Python project that does not use "app-layout" but instead
...
class ConfigDiscovery:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        package_dir.pop("", None)  # This falls under the "app-layout" umbrella
    ...
    def _analyse_src_layout(self) -> bool:
        """Try to find all packages or modules under the ``app`` directory
        (or anything pointed by ``package_dir[""]``).

        The "app-layout" is relatively safe for automatic discovery.
        We assume that everything within is meant to be included in the
        distribution.

        If ``package_dir[""]`` is not given, but the ``app`` directory exists,
        this function will set ``package_dir[""] = "app"``.
        """
        package_dir = self._package_dir
        src_dir = os.path.join(self._root_dir, package_dir.get("", "app"))
        if not os.path.isdir(src_dir):
            return False

        log.debug(f"`app-layout` detected -- analysing {src_dir}")
