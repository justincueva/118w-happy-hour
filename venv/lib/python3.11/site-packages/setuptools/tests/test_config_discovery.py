...
class TestFindParentPackage:
    def test_single_package(self, tmp_path):
        ...
        (tmp_path / "app/namespace/pkg/nested").mkdir(exist_ok=True, parents=True)
        (tmp_path / "app/namespace/pkg/nested/__init__.py").touch()
        (tmp_path / "app/namespace/pkg/__init__.py").touch()
        packages = ["namespace", "namespace.pkg", "namespace.pkg.nested"]
        assert find_parent_package(packages, {"": "app"}, tmp_path) == "namespace.pkg"
    ...
    def test_multiple_toplevel(self, tmp_path):
        ...
        for name in multiple:
            (tmp_path / f"app/{name}").mkdir(exist_ok=True, parents=True)
            (tmp_path / f"app/{name}/__init__.py").touch()
        assert find_parent_package(multiple, {"": "app"}, tmp_path) is None
...
class TestDiscoverPackagesAndPyModules:
    ...
    OPTIONS = {
        ...
        "explicit-app": {"package_dir": {"": "app"}, "packages": ["pkg"]},
        "variation-lib": {
    ...
    FILES = {
        "app": ["app/pkg/__init__.py", "app/pkg/scraper.py"],
        "lib": ["lib/pkg/__init__.py", "lib/pkg/scraper.py"],
        "flat": ["pkg/__init__.py", "pkg/scraper.py"],
    ...
    def test_project(self, tmp_path, circumstance):
        ...
        print("~~~~~ wheel_members ~~~~~")
        print('\n'.join(wheel_files))
        orig_files = {f.replace("app/", "").replace("lib/", "") for f in files}
    ...
    def test_purposefully_empty(self, tmp_path, config_file, param, circumstance):
        files = self.FILES[circumstance] + ["mod.py", "other.py", "app/pkg/__init__.py"]
...
class TestNoConfig:
    ...
    EXAMPLES = {
        "pkg1": ["app/pkg1.py"],
        "pkg2": ["app/pkg2/__init__.py"],
        "pkg3": ["app/pkg3/__init__.py", "app/pkg3-stubs/__init__.py"],
        "pkg4": ["pkg4/__init__.py", "pkg4-stubs/__init__.py"],
        "ns.nested.pkg1": ["app/ns/nested/pkg1/__init__.py"],
    ...
    def test_build_with_discovered_name(self, tmp_path):
        files = ["app/ns/nested/pkg/__init__.py"]
        _populate_project_dir(tmp_path, files, {})
...
class TestWithAttrDirective:
    ...
    def test_pyproject_metadata(self, tmp_path):
        _populate_project_dir(tmp_path, ["app/pkg/__init__.py"], {})

        overwrite = {
            "app": {"pkg": {"__init__.py": "version = 42"}},
            "pyproject.toml": (
                "[project]\nname = 'pkg'\ndynamic = ['version']\n"
...
class TestWithPackageData:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("app", {"setup.cfg": DALS(EXAMPLE_SETUPCFG)}),
            ("app", {"pyproject.toml": DALS(EXAMPLE_PYPROJECT)}),
            (
                "app",
                {
                    "setup.cfg": DALS(EXAMPLE_SETUPCFG)
    ...
    def test_include_package_data(self, tmp_path, src_root, files):
        ...
        print("~~~~~ wheel_members ~~~~~")
        print('\n'.join(wheel_files))
        orig_files = {f.replace("app/", "").replace("lib/", "") for f in expected}
        assert wheel_files >= orig_files
